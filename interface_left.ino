...
String calibracao_A = "00.000";                 // armazena o resultado da calibracao das bombas
String calibracao_B = "00.000";
...
float converterMLParaPassos(float fator_calibracao_seringa, int fluxo);
void enviarFluxo();
...
void menuSelecionarBomba() // menu 1
{
    ...
    // ********************************************************************************
    //Serial.println(pos_seta_selecao);
    // ********************************************************************************
...
void calibrarBomba() // menu 8
{
    ...
    if      (bomba_selecionada == 'A') fluxo_A = "0000";
    else if (bomba_selecionada == 'B') fluxo_A = "0000";
...
void menuSelecionarCalibracao()  // menu 9
{
    ...
    {                                                       // sim...
        ...
        if (posicao_algarismo == 3) {
            ...
            {
                calibracao_A = fluxo_A.toFloat()*(calibracao_selecionada.toInt()/minutos_calibracao_int);
            }
            else if (bomba_selecionada == 'B')
            {
                calibracao_B = fluxo_B.toFloat()*(calibracao_selecionada.toInt()/minutos_calibracao_int);
            } 
...
float converterRPMParaPassos(int fluxo) 
{
    return round( (((fluxo*PASSOS_POR_REVOLUCAO)/60)*100.0)/100.0 );
}
...
float converterMLParaPassos(float fator_calibracao_seringa, int fluxo) 
{ 
    return round( ((fator_calibracao_seringa*fluxo)*100.0)/100.0 );
}
...
void enviarFluxo()
{
    ...
    float pps_enviado_A;
    if      (unidade_A == 1) pps_enviado_A = converterMLParaPassos(calibracao_A.toFloat(), fluxo_A.toFloat());
    else if (unidade_A == 2) pps_enviado_A = fluxo_A.toFloat() * 1.00;
    else if (unidade_A == 3) pps_enviado_A = converterRPMParaPassos(fluxo_A.toFloat());

    float pps_enviado_B;
    if      (unidade_B == 1) pps_enviado_B = converterMLParaPassos(calibracao_B.toFloat(), fluxo_B.toFloat());
    else if (unidade_B == 2) pps_enviado_B = fluxo_B.toFloat() * 1.00;
    else if (unidade_B == 3) pps_enviado_B = converterRPMParaPassos(fluxo_B.toFloat());          
    
    // junta em uma unica mensagem
    String fluxo_enviado = String(pps_enviado_A) + "|" + String(pps_enviado_A) + "\n";
